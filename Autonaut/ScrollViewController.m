...
- (void)viewDidLoad
{
    ...
    automata = [[Automata alloc] initwithRule:[rule integerValue] randomInitials:[random boolValue] width:self.view.bounds.size.width*3*.99999 height:self.view.bounds.size.height*.99999];
    automataView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width*3, self.view.bounds.size.height)];
    [automataView setImage:[automata GIFImageFromDataWithScale:1.0]];
